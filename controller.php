...
if(isset($_POST['resetPassword'])){
    ...
    $confirmPassword = md5($_POST['confirmPassword']);
    $psdQuery = "SELECT * FROM usersInfo WHERE email = '$email' AND password = '$oldPassword'";
    $psdQueryRun = mysqli_query($conn, $psdQuery);

    if($psdQueryRun){
        if(mysqli_num_rows($psdQueryRun) > 0){
            if(strlen($_POST['newPassword']) < 8){
                $errors['passwordLengthError'] = 'Use 8 or more characters with a mix of letters, numbers & symbols';
            }else{
                if($newPassword === $confirmPassword){
                    // Update Password
                    $updatePsdQuery = "UPDATE usersInfo SET password = '$newPassword' WHERE email = '$email'";
                    $runUpdatePsdQuery = mysqli_query($conn, $updatePsdQuery);
                    
                    if($runUpdatePsdQuery){
                        $_SESSION['message'] = "Password Changed";
                    }else{
                        $errors['passwordError'] = "Password Not Updated";
                    }
                }else{
                    $errors['passwordNotMatched'] = "New Password Not Matched";
                }
            }
        }else{
            $errors['oldPasswordError'] = "Old Password Not Matched";
        }
    }else{
        $errors['db-error'] = "Failed While Checking Password From database!";
    }
}